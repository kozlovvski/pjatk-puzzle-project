...
//intialize data
const nodes: InitialNode[] = new Array(+MERGED_INITIAL_SETTINGS_VALUES.nodesNumber)
  .fill(0)
  .map((_, index) => ({ index }));
...
const getTargetIndex = (sourceIndex: number) => (links as unknown as ModifiedLink[]).find(link => link.source.index == sourceIndex)?.target.index
...
const nodeCircles = (svg
  ...
  .call(
    d3.drag<SVGCircleElement, ModifiedNode>().on("start", dragstarted).on("drag", dragged).on("end", dragended)
  )
